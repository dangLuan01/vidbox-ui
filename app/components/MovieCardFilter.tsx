...
export default function MovieCardFilter({ movie }: { movie: Movie }) {
  ...
  return (
    ...
    <Link href={`/${movie.media_type}/${movie.id}`}>
        <div className="relative overflow-hidden rounded-md hover:text-white aspect-[2/3]">
          <img
            ...
            decoding="async"
            fetchPriority="high"
            className="transition-opacity duration-300 opacity-100" width="300" height="450" style={{objectFit: "cover"}}
          />
    ...
    {hovered && (
        <div className={`absolute top-0 ${side === "right" ? "left-full ml-0.5" : "right-full mr-2"} z-50 w-[370px] rounded-xl bg-black shadow-2xl ring-1 ring-white/10`} >
          ...
          <div className="p-2">
            ...
            <div className="flex items-center gap-2">
                <button
                  onClick={() => router.push(`/review/${movie.media_type}/${movie.id}`)}
